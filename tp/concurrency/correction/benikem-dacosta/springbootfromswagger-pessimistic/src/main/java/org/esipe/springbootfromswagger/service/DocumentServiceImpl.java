...
public class DocumentServiceImpl {
    ...
    public Document updateById(Document document, String documentId) {
        try {
            log.info("Processing : resource is now locked, lock : " + findById(documentId).getLock());
            Thread.sleep(5000);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public Lock putDocumentLock(Lock lock, String documentId) {
        Document document = findById(documentId);
        while(this.getDocumentLock(documentId) != null) {
            try {
                log.info("Existing lock, waiting for release... lock : " + findById(documentId).getLock() );
                Thread.sleep(1000);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void deleteDocumentLock(String documentId) {
        ...
        if (document.getLock() != null) {
            ...
            log.info("Lock released, lock : " + findById(documentId).getLock());
        }
    }
}
